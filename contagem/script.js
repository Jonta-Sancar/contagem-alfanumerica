...
function randomValue(type, value1, value2, callback){
  ...
  while(!finish){
    ...
    if(response >= min_random_val && response <= max_random_val){
      if(convert){
        response = this.convertValue(type, random_val);
      }

      if(response.length == value1){
...
function alNum (METHOD_, VALUES_, RETURN_, CALLBACK_) {
  ...
  if(METHOD_.toLowerCase() == 'calc') {
    ...
    if(VALUES_.length == 2){
      response = alnum.randomValue(VALUES_[0], VALUES_[1]);
    } else if(VALUES_.length > 2){
      response = alnum.randomValue(VALUES_[0], VALUES_[1], VALUES_[2]);
    } else {
      response = false;
    }
